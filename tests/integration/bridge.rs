...
// =============================================================================
// Busbar Authentication Integration Tests
// =============================================================================

#[tokio::test]
#[cfg(feature = "busbar")]
#[ignore] // Run with --ignored when environment is properly configured
async fn test_bridge_with_busbar_auth_from_env() {
    use busbar_sf_bridge::BusbarAuthConfig;
    
    // This test requires SF_ACCESS_TOKEN and SF_INSTANCE_URL environment variables
    // to be set. In CI/CD, these are typically set by the ephemeral session.
    let config = BusbarAuthConfig::new();
    let wasm_bytes = load_test_wasm_bytes().expect("WASM plugin not found");
    
    let bridge = busbar_sf_bridge::SfBridge::new_with_busbar_auth(wasm_bytes, config)
        .await
        .expect("Failed to create bridge with Busbar auth");
    
    // Verify the bridge can execute a simple query
    let input = serde_json::json!({
        "soql": "SELECT Id FROM Account LIMIT 1"
    });
    ...
    assert!(
        result["success"].as_bool().unwrap_or(false),
        "Query with Busbar auth should succeed"
    );
}

#[tokio::test]
#[cfg(feature = "busbar")]
#[ignore] // Run with --ignored when JWT credentials are available
async fn test_bridge_with_busbar_auth_jwt() {
    use busbar_sf_bridge::{BusbarAuthConfig, JwtAuthConfig};
    
    // This test requires JWT credentials to be configured.
    // In a real environment, these would come from secure storage.
    let jwt_config = JwtAuthConfig::with_key_file(
        std::env::var("SF_CONSUMER_KEY").expect("SF_CONSUMER_KEY not set"),
        std::env::var("SF_USERNAME").expect("SF_USERNAME not set"),
        std::env::var("SF_PRIVATE_KEY_PATH").expect("SF_PRIVATE_KEY_PATH not set"),
    ...
    let bridge = busbar_sf_bridge::SfBridge::new_with_busbar_auth(wasm_bytes, config)
        .await
        .expect("Failed to create bridge with JWT auth");
    ...
    assert!(
        result["success"].as_bool().unwrap_or(false),
        "Query with JWT auth should succeed"
    );
}
